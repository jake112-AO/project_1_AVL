...
Node* AVLTree::removeHelper(Node* node, string id) {
    if (node == nullptr) {
        return node;
    }
    if (id < node->ufid) {
        ...
        if (node->left == nullptr || node->right == nullptr) {
            ...
            if (temp == nullptr) {
                ...
                *node = *temp; // copy all data from fields temp to *node
            }
            delete temp;
        } else {
...
Node* AVLTree::findNode(Node* node, const string& id) {
    if (node == nullptr || node->ufid == id) {
        return node;
    }
    if (id < node->ufid) {
